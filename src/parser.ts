...
export class Parser {
    ...
    buildLogFile: any
    linterLog = []
    compileDiagnostics = vscode.languages.createDiagnosticCollection('LaTeX')
    ...
    showCompilerDiagnostics(createBuildLogRaw: boolean = false) {
        ...
        if (this.buildLogFile)
            this.compileDiagnostics.set(vscode.Uri.file(this.buildLogFile.name),
        ...
        for (let file in diagsCollection)
            this.compileDiagnostics.set(vscode.Uri.file(file), diagsCollection[file])
    }
